...
const hasAvailableSlots = async (date: Date, allBookings: Map<string, string[]>) => {
  ...
  // Obtenir les créneaux disponibles pour ce jour (selon la config)
  const availableTimeSlots = generateTimeSlotsForDay(dayOfWeek)
  ...
  bookings.forEach(time => {
    ...
    blockedSlots.add(`${hour.toString().padStart(2, '0')}:${minute.toString().padStart(2, '0')}`)
    const nextMinutes = startMinutes + 30
    const nextHour = Math.floor(nextMinutes / 60)
    const nextMinute = nextMinutes % 60
    if (nextHour < 18) {
      blockedSlots.add(`${nextHour.toString().padStart(2, '0')}:${nextMinute.toString().padStart(2, '0')}`)
    }
  })
  ...
  if (isToday) {
    ...
    for (const slot of availableTimeSlots) {
      ...
      if (slotTime > currentTime && !blockedSlots.has(slot)) {
  ...
  const availableSlotsCount = availableTimeSlots.filter(slot => !blockedSlots.has(slot)).length
...
export async function GET(request: NextRequest) {
  try {
    ...
    while (currentDate <= endDate) {
      ...
      // Vérifie : jours bloqués (lundi, mercredi, dimanche, samedi) + semaines alternées
