// lib/services/reminder.ts
// Système de rappel par email 24h avant le rendez-vous

import { prisma } from '@/lib/prisma'
import { createTransporter } from './email'

// Interface pour les données de rappel
...
export const sendReminderEmail = async (booking: ReminderData) => {
...
export const getBookingsToRemind = async () => {
...
// Fonction pour vérifier si un rappel a déjà été envoyé
export const hasReminderBeenSent = async (bookingId: string) => {
  // Pour l'instant, on considère qu'on peut envoyer un rappel par jour
  // Dans une version plus avancée, on pourrait stocker les rappels envoyés
  return false
}

// Fonction pour marquer un rappel comme envoyé
export const markReminderAsSent = async (bookingId: string) => {
  // Dans une version plus avancée, on pourrait stocker les rappels envoyés
  // Pour l'instant, on ne fait rien
  console.log(`📧 Rappel marqué comme envoyé pour la réservation ${bookingId}`)
}
